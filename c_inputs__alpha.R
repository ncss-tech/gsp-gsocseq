...
# GSOC ----
# setwd("D:/geodata/project_data/gsp-gsocseqwp")
gsoc  <- raster("D:\\GIS\\WORLD\\R\\GSOCmap1.5.0.tif")
...
# lf <- list.files()
# idx <- grepl("_Stack_228_....._TC.tif", lf)
# file.remove(lf[idx])

# tmp8100
gdalUtilities::gdalwarp(
  srcfile = paste0(aoi, "_AverageTemperature_1981-2001.tif"),
  dstfile = paste0(aoi, "_Temp_Stack_228_81-00_TC.tif"),
  te      = c(bbox(gsoc2)),
  tr      = res(gsoc2),
  r       = "bilinear",
  srcnodata = -3.4e+38,
...
gdalUtilities::gdalwarp(
  ...
  dstfile = paste0(aoi, "_Temp_Stack_228_01-19_TC.tif"),
  te      = c(bbox(gsoc2)),
  tr      = res(gsoc2),
  r       = "bilinear",
  srcnodata = -3.4e+38,
...
gdalUtilities::gdalwarp(
  ...
  dstfile = paste0(aoi, "_Prec_Stack_228_81-00_TC.tif"),
  te      = c(bbox(gsoc2)),
  tr      = res(gsoc2),
  r       = "bilinear",
  srcnodata = -3.4e+38,
...
gdalUtilities::gdalwarp(
  ...
  dstfile = paste0(aoi, "_Prec_Stack_228_01-19_TC.tif"),
  te      = c(bbox(gsoc2)),
  tr      = res(gsoc2),
  r       = "bilinear",
  srcnodata = -3.4e+38,
...
##  convert variables ----
aoi <- "CONUS"
tmp8100 <- rast(paste0(aoi, "_Temp_Stack_228_81-00_TC.tif"))
tmp0119 <- rast(paste0(aoi, "_Temp_Stack_228_01-19_TC.tif"))
...
tmp8100 <- rast(paste0(aoi, "_Temp_Stack_228_81-00_TC.tif"))
tmp0119 <- rast(paste0(aoi, "_Temp_Stack_228_01-19_TC.tif"))
ppt8100 <- rast(paste0(aoi, "_Prec_Stack_228_81-00_TC.tif"))
ppt0119 <- rast(paste0(aoi, "_Prec_Stack_228_01-19_TC.tif"))
pet8100 <- rast(paste0(aoi, "_PET_Stack_228_81-00_TC.tif"))
pet0119 <- rast(paste0(aoi, "_PET_Stack_228_01-19_TC.tif"))
...
tmp8100_avg_yr <- avgRS(tmp8100, "tmmx", yrs8100)
tmp0119_avg_yr <- avgRS(tmp0119, "tmmx", yrs0119)
ppt8100_sum_yr <- sumRS(ppt8100, "pr",   yrs8100)
ppt0119_sum_yr <- sumRS(ppt0119, "pr",   yrs0119)
pet8100_avg_yr <- avgRS(pet8100, "pet",  yrs8100)
pet0119_avg_yr <- avgRS(pet0119, "pet",  yrs0119)
...
ppt8100_sum_yr <- rast(paste0(aoi, "_Prec_Stack_81-00_TC_yr.tif"))
ppt0121_sum_yr <- rast(paste0(aoi, "_Prec_Stack_01-19_TC_yr.tif"))
...
npp8100_ppt <- 3000 * (1 - exp(-0.000664 * ppt8100_sum_yr))
...
ppt8100_min_sum <- ppt8100_sum_yr * 0.95

npp8100_min_sum_ppt <- 3000 * (1 - exp(-0.000664     * ppt8100_min_sum))
npp8100_min_avg_tmp <- 3000 / (1 + exp(1.315 - 0.119 * tmp8100_min_avg))

npp8100_min <- lapply(1:20, function(i) {
  cat("calculating min from ", i, "\n")
  npp <- min(npp8100_min_sum_ppt[[i]], npp8100_min_avg_tmp[[i]], na.rm = TRUE)
})
...
ppt8100_max_sum <- ppt8100_sum_yr * 1.05

npp8100_max_sum_ppt <- 3000 * (1 - exp(-0.000664     * ppt8100_max_sum))
npp8100_max_avg_tmp <- 3000 / (1 + exp(1.315 - 0.119 * tmp8100_max_avg))

npp8100_max <- lapply(1:20, function(i) {
  cat("calculating min from ", i, "\n")
  npp <- min(npp8100_max_sum_ppt[[i]], npp8100_max_avg_tmp[[i]], na.rm = TRUE)
})
...
lc <- rast("D:\\GIS\\WORLD\\R\\GlcShare_v10_Dominant\\glc_shv10_DOM.Tif")
...
su_rs <- rast(su_rs)
writeRaster(su_rs, filename = "Stack_Set_SPIN_UP_AOI.tif", filetype = "GTiff", overwrite = TRUE)
...
vars <- c(SOC  = paste0(aoi, "_GSOCmap1.5.0.tif"),
          ...
          LU   = paste0(aoi, "_glc_shv10_DOM.tif"),
          DR   = paste0(aoi, "_glc_shv10_DOM_DR.tif"),
          TEMP = paste0(aoi, "_Temp_Stack_228_01-19_TC.tif"),
          PREC = paste0(aoi, "_Prec_Stack_228_01-19_TC.tif"),
...
fr_rs <- rast(fr_rs)
# writeRaster(fr_rs, filename = "Stack_Set_FOWARD.tif", filetype = "GTiff", overwrite = TRUE)
...
pts <- rast("CONUS_glc_shv10_DOM.tif")
pts <- pts * 1

terra::writeRaster(pts, "CONUS_glc_shv10_DOM_d.tif", overwrite = TRUE)
pts <- raster(pts)
pts <- rasterToPoints(pts, spatial = TRUE) 

head(pts@data)

pts$CONUS_glc_shv10_DOM <- as.character(pts$LU)
nrow(pts)
