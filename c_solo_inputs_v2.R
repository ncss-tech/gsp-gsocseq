...
wdir <- ("D:/geodata/project_data3/gsp-gsocseqsolo")
setwd(wdir)
...
gsoc  <- raster("D:\\GIS\\PROJECT_21\\GSOCS\\R\\GSOCmap1.5.0.tif")
gsoc2 <- crop(gsoc, aoi)
...
clay2 <- raster(clay_src[2])
clay3 <- raster(clay_src[3])
clay4 <- raster(clay_src[4])

clay1_AR<-crop(clay1,gsoc2)
clay2_AR<-crop(clay2,gsoc2)
clay3_AR<-crop(clay3,gsoc2)
clay4_AR<-crop(clay4,gsoc2)
...
clay_2 <- crop(Clay_WA, aoi)
clay_3 <- mask(clay_2, aoi)
clay_4 <- resample(clay_3, gsoc2, method = "bilinear", progress = "text")
setwd(wdir)
writeRaster(clay_4,filename= "CONUS_gnatsgo_fy20_1km_clay_wt.tif",format='GTiff', overwrite = TRUE)
clay <- raster("CONUS_gnatsgo_fy20_1km_clay_wt.tif")

rm(list = c('clay1', 'clay2', 'clay3', 'clay4', 'clay1_AR', 'clay2_AR', 'clay3_AR', 'clay4_AR', 'clay_2', 'clay_3', 'clay_4'))
rm(Clay_WA)
# TerraClimate ----
...
tmp0121_avg <- avgRS(tmp0121, var = "tmmx", mos0119) * 0.1
...
tmp0121_avg_yr <- avgRSyr(tmp0121, "tmmx", yrs0119) * 0.1
ppt8100_avg_yr <- avgRSyr(ppt8100, "pr", yrs8100)
ppt0121_avg_yr <- avgRSyr(ppt0121, "pr", yrs0119)
pet8100_avg_yr <- avgRSyr(pet8100, "pet", yrs8100) * 0.1
pet0119_avg_yr <- avgRSyr(pet0119, "pet", yrs0119) * 0.1
...
# Vegetation Cover ----
# vars <- paste0("NDVI_2000-20_prop_gt_06_CR_MES_", formatC(1:12, width = 2, flag = "0"), "_conus.tif")

vars <- paste0("M", formatC(1:12, width = 3, flag = "0"), "_NDVI_2000-2019_prop_gt_06_CR_MES_01.tif")
# vars2 <- list.files(".", pattern = "_CR_MES_01.tif$")               
...
lc2 <- resample(lc2, gsoc2, method = "ngb", progress = "text", datatype = "INT1S")
...
# Stack layers ----
# wdir <- ("D:/geodata/project_data2/gsp-gsocseqwp")
# setwd(wdir)
...
#write_sf(st_as_sf(su_pts), dsn = "pre-test", driver = "ESRI Shapefile")
su_df  <- as.data.frame(su_pts)

# su_pts2 <- su_pts[as.character(su_df$LU) %in% c(2, 3, 5, 12, 13), ]
# su_pts2 <- su_pts[
...
pts <- subset(pts, !CONUS_glc_shv10_DOM %in% c(11))
