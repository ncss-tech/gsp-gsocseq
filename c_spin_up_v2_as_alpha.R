...
rothC_r <- as.data.frame(
  cbind(su_df[c("cell", "x", "y", "LU", "DR")], SOC = SOC_r, FallIOM = FallIOM_r, pClay = pClay_r,
        source = "r",
        do.call(
          "rbind", 
...
rothC_min <- as.data.frame(
  cbind(su_df[c("cell", "x", "y", "LU", "DR")], SOC = SOC_min, FallIOM = FallIOM_min, pClay = pClay_min, 
        source = "min",
        do.call(
          "rbind", 
...
rothC_max <- as.data.frame(
  cbind(su_df[c("cell", "x", "y", "LU", "DR")], SOC = SOC_max, FallIOM = FallIOM_max,  pClay = pClay_max, 
        source = "max",
        do.call(
          "rbind", 
...
lu  <- lu[values(lu) %in% c(2, 3, 5, 12, 13), ]
gsoc <- gsoc * lu
