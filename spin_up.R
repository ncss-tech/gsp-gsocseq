...
Vector <- as(su_pts, "SpatialPoints")


# Create A vector to save the results
C_INPUT_EQ <- su_pts[1:200, ]
...
DR_im<-Vector_variables[[41]]

LU_im<-Vector_variables[[40]]
...
Roth_C <- function(
  ...
  # P = (Precip[,2]); E=(Evp[,2]); S.Thick = soil.thick; pClay = clay; pE = 1; bare=bare1
  ...
  xi.frame=data.frame(years, xi = rep(fT*fW_2*fC*fPR,length.out=length(years)))
...
for (i in 1:200) {
  ...
  if (any(is.na(Evp[,2])) | any(is.na(Temp[,2])) | any(is.na(SOC_im[i])) | any(is.na(clay_im[i])) | any(is.na(Precip[,2]))  |  any(is.na(Cov2[,2]))  |  any(is.na(Cov1[,1]))  | any(is.na(DR_im[i])) |  (SOC_im[i]<0) | (clay_im[i]<0) ) {C_INPUT_EQ[i,2]<-0}else{
    ...
    # Cinputs=b; years=years; DPMptf=0; RPMptf=0; BIOptf=0; HUMptf=0; FallIOM=FallIOM; Temp=Temp; Precip=Precip; Evp=Evp; Cov=Cov; Cov1=Cov1; Cov2=Cov2; soil.thick=soil.thick; SOC=SOC; clay=clay; DR=DR; bare1=bare1; LU=LU
